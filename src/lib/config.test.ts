...
test('loadConfig returns defaults when env vars are absent', () => {
  ...
  assert.equal(path.isAbsolute(config.dbPath), true);
  assert.equal(path.isAbsolute(config.workspaceRoot), true);
});
...
test('loadConfig parses explicit env overrides', () => {
  const config = loadConfig({
    HOST: '0.0.0.0',
    PORT: '8080',
    MISSION_CONTROL_DB_PATH: './tmp/test.sqlite',
  ...
  assert.equal(config.dbPath.endsWith(path.join('tmp', 'test.sqlite')), true);
  assert.equal(config.workspaceRoot.endsWith(path.join('tmp', 'workspace')), true);
});
